...
- **Gear Drops 2026-06-26 — admin review gate on auto-published mementos**:
  - **`20260626_gear_drops_memento_admin_review.sql`** — Adds `trip_reports.gd_review_status text check ('pending','approved','rejected')` + `gd_reviewed_at` + `gd_reviewed_by` (with partial index `where kind='gear_drop_run'`). Backfills all existing published gear_drop_run rows to `'approved'` so historical mementos stay visible. Re-defines `gear_drop_advance_run` so memento auto-publish now lands in `gd_review_status='pending'` (everything else identical to the per-waypoint-notif-removal version). Adds two admin-only RPCs: `admin_approve_gear_drop_memento(p_run_id)` and `admin_reject_gear_drop_memento(p_run_id)` — both gated on `public.is_admin(auth.uid())`. **Winner declaration is still automatic** — first finisher claims the win. If admin rejects the memento, the winner declaration doesn't auto-revoke (separate workflow TBD). **RLS unchanged** — the third trip_reports SELECT branch (drop status past draft) still lets the event-page recap query fetch pending rows so the placeholder card can render. Client gates the OPEN action (RACER RECAPS card masks hero + non-tappable for non-admin until approved; `GearDropMementoScreen` shows an "Awaiting admin review" overlay for non-admin non-owner viewers of pending/rejected runs). Owner + admin bypass both gates. Admin gets inline APPROVE / REJECT buttons on pending recap cards.
