...
| `ambassador_orders` | Raw Shopify orders that used an ambassador's discount code, ingested via Phase 1B webhook. `subtotal`, `commission_eligible_subtotal` (subtotal minus excluded product_types), `classification ∈ {unclassified, deposit, confirmation_part, walk_in_part, excluded}`. Stores `raw_webhook` jsonb for debugging. RLS: own ambassador SELECT + admin SELECT. Writes via service role only | `ambassador_id` → `ambassadors(id)` CASCADE, `journey_id` → `ambassador_journeys(id)` SET NULL |
...
| `ambassador_payouts` | One row per Stripe payout. `period_start/end`, `gross_amount`, `clawback_amount`, `net_amount`, `scheduled_for` (last business day of next month). Phase 2 populates. RLS: own ambassador SELECT + admin SELECT/UPDATE | `ambassador_id` → `ambassadors(id)` RESTRICT |
...
### Shopify ambassador integration — `shopify-create-discount-code` / `shopify-webhook` / `shopify-backfill-orders` / `shopify-register-webhooks`
...
**Phase 1B.5/.6 still deferred**: multi-ambassador conflict queue UI (webhook flags `is_conflict=true` but admin can't resolve yet); refund/clawback (`orders/refunded` topic registered but webhook handler ignores it).
...
- `admin_ambassador_commission_by_month(p_months int default 6)` — time-series of commission per confirmation-month for bar chart; empty months returned as 0
