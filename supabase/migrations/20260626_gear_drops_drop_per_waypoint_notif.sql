-- ============================================================================
-- GEAR DROPS — drop per-waypoint gear_drop_unlock notification
-- ============================================================================
-- Per-waypoint push notifications were noise: every submission fired a
-- self-notification ("Stop 2 of 5 complete · 3 to go") that the racer was
-- already seeing live in-app. Each insert triggered notifications_send_push,
-- so racers got a buzz on every waypoint.
--
-- This migration re-defines gear_drop_advance_run to NOT insert a
-- gear_drop_unlock notification on intermediate waypoints. The final-pin
-- branch is unchanged — winner gets gear_drop_won, fellow racers get
-- gear_drop_winner, both still fire pushes (that's the only push gear drops
-- need).
--
-- Phase 5 SELECT-policy widening + memento auto-publish + backfill are all
-- preserved verbatim — only the else-branch notif insert is removed.
--
-- The 'gear_drop_unlock' value stays in the notifications.type CHECK
-- constraint so any historical rows continue to render correctly in
-- bell history. We just stop producing new ones.
--
-- Idempotent.
-- ============================================================================
...
begin
  ...
  -- Actor identity — still needed for the final-pin winner branch below.
  begin
  ...
  if v_is_last then
    ...
    -- ─── Winner claim (atomic) — first finisher only ───────────────────
  ...
  -- NOTE: intermediate waypoints used to insert a self gear_drop_unlock
  -- notification here. Removed 2026-06-26 — see migration header.
