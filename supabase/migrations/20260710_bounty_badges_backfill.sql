-- 2026-07-10: One-shot backfill for bounty badges.
--
-- The `bounty_submissions_recompute_badges` trigger + updated
-- `recompute_badges` walker were shipped in
-- `20260701_badges_bounty_hunter_demo_specialist.sql`, but that migration
-- was never applied in this environment (verified 2026-07-10 by the
-- 42883 "recompute_badges_on_bounty_approve() does not exist" error).
-- This migration replays the necessary chunks of that Phase 3 migration
-- so the trigger exists AND the walker covers bounty tiers, then runs a
-- backfill for every user with an approved submission.
--
-- Idempotent throughout: `create or replace` on functions,
-- `drop ... if exists` on the trigger, and `on conflict (user_id,
-- category, tier_index) do nothing` inside the walker means re-runs are
-- no-ops.

-- ── recompute_badges walker (Phase 3 version — includes bounty tiers) ──
...
-- ── get_badge_progress (Phase 3 version — includes bounty categories) ──
...
-- ── Backfill: walk badges for every user with an approved submission ──
do $$
declare
  v_uid uuid;
begin
  ...
  loop
    ...
    exception when others then
      raise notice 'recompute_badges backfill failed for %: %', v_uid, sqlerrm;
    end;
  end loop;
end
$$;
