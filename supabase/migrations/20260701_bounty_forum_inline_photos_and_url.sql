-- ─────────────────────────────────────────────────────────────────────────────
-- admin_approve_bounty (forum_thread destination) — inline photos + URL type
-- ─────────────────────────────────────────────────────────────────────────────
-- Follow-up to the section-walker rewrite. Two additions here:
--
--   1. Photos section: previously dumped ALL photos to the row's `photos`
--      jsonb column (which powers the card thumbnail + OG preview) and did
--      NOT emit them inline. Now the walker emits every photo as a
--      `<figure><img alt=""><figcaption>caption</figcaption></figure>`
--      block INTO the current section's body — so if the participant put a
--      Photos block under Paragraph 1, the images render right under
--      Paragraph 1 on the published thread. The row's `photos` jsonb still
--      gets the first photos block for the card thumbnail + OG.
--
--   2. New `url` section type: emits SEO-safe outbound links using
--      `<a href="..." rel="noopener noreferrer nofollow" target="_blank">…</a>`.
--      Draft value is a plain URL string; section.label becomes the link
--      text (falling back to the URL itself).
--
-- SEO-optimized HTML emitted:
--   • `<h2>` for subheadings (semantic hierarchy — the title is the page h1)
--   • `<p>` for paragraphs
--   • `<figure><img alt="..." loading="lazy" /><figcaption>...</figcaption></figure>`
--     for images (semantic image + caption pairing)
--   • `<ul><li>` for bullet lists (semantic lists, better than raw <br>)
--   • `<a rel="noopener noreferrer nofollow" target="_blank">` for external
--     links (safety + prevents PageRank leaks from user-submitted content)
--
...
begin
  ...
  if v_destination = 'trip_report' then
    ...
    -- Hero image URL (used as card thumbnail if no photos section)
    for v_section in select * from jsonb_array_elements(v_sections)
    loop
      if (v_section ->> 'type') = 'hero_image' then
        v_hero_url := (v_sub.draft -> (v_section ->> 'id')) ->> 'url';
    ...
    -- First photos block goes to forum_threads.photos for the card thumbnail
    -- + OG image. NOTE: we still emit ALL photos inline via the section walk
    -- below; this is JUST for the row-level jsonb.
    for v_section in select * from jsonb_array_elements(v_sections)
    loop
      if (v_section ->> 'type') = 'photos'
         and jsonb_typeof(v_sub.draft -> (v_section ->> 'id')) = 'array' then
    ...
    -- ── Section walk: every content type is handled, including inline
    --    photos (as <figure>+<figcaption>) and URL links. ──
    ...
    loop
      ...
      -- title + hero_image + route_builder are surfaced through dedicated
      -- columns, not inline body.
      ...
      -- Otherwise emit HTML for this section into the current body.
      v_val := null;
      if (v_section ->> 'fixed') = 'true' then
      ...
      if v_st = 'p' then
        ...
        -- Inline photo gallery: <figure> per photo with <figcaption> for
        -- captions. Renders in the same order participants uploaded them,
        -- IN THE POSITION the admin placed the block. Skips _uploading
        -- placeholders and entries without a resolvable URL.
        v_arr := v_sub.draft -> (v_section ->> 'id');
        if jsonb_typeof(v_arr) = 'array' and jsonb_array_length(v_arr) > 0 then
          select string_agg(
                   '<figure>'
        ...
        -- SEO-safe outbound link. Section label becomes the link text; if
        -- empty, we surface the URL itself. rel="noopener noreferrer nofollow"
        -- + target="_blank" is the safe default for user-submitted links.
        v_label := coalesce(nullif(v_section ->> 'label', ''), '');
        if v_val is not null and length(trim(v_val)) > 0
           and (v_val like 'http://%' or v_val like 'https://%') then
          v_html := '<p>';
          if length(v_label) > 0 then
            v_html := v_html || '<strong>' || v_label || ':</strong> ';
          end if;
          v_html := v_html || '<a href="'
                    || replace(replace(v_val, '"', '&quot;'), '<', '&lt;')
                    || '" rel="noopener noreferrer nofollow" target="_blank">'
                    || case when length(v_label) > 0 then v_label
                            else replace(replace(replace(v_val, '&', '&amp;'), '<', '&lt;'), '>', '&gt;') end
                    || '</a></p>';
        end if;
      end if;
    ...
    -- render paths + text search / OG scraping.
    v_concat_body := '';
