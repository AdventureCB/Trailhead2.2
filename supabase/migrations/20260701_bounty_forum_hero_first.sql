-- ─────────────────────────────────────────────────────────────────────────────
-- admin_approve_bounty (forum_thread destination) — hero_image wins as card
-- ─────────────────────────────────────────────────────────────────────────────
-- Bug: the previous INSERT preferred `v_photos` (first photos block) over
-- `v_hero_url` (dedicated hero_image block) when populating
-- `forum_threads.photos`. Since the forum article renders `photos[0]` as
-- the hero banner (see ForumScreen render at line ~8391), the participant's
-- first uploaded photo/video was replacing the intentional hero image the
-- admin/participant selected.
--
-- Fix: hero_image URL wins first when present. Falls back to the first
-- photos block only if no hero_image was set. Inline photos are still
-- emitted in the article body via <figure> walks, so nothing is dropped.
--
...
begin
  ...
  if v_destination = 'trip_report' then
    ...
    insert into public.forum_threads (
      ...
      v_thread_sections,
      -- Hero image wins the card thumbnail + article hero slot. Falls back
      -- to the first photos block only if no hero_image was set. Inline
      -- photos are still emitted in the article body via <figure> walks,
      -- so nothing is lost.
