-- ─────────────────────────────────────────────────────────────────────────────
-- admin_approve_bounty (forum_thread destination) — inline video support
-- ─────────────────────────────────────────────────────────────────────────────
-- The previous photos-section walker filtered out video entries entirely
-- (`coalesce(elem ->> 'type', 'image') <> 'video'`), which dropped every
-- video-typed entry the participant uploaded. Now video entries emit as
-- `<figure><video>...</video><figcaption>...</figcaption></figure>` blocks
-- inline where the Photos section sits, just like images.
--
-- Video tag attrs (SEO + UX friendly):
--   • controls        — playback UI
--   • preload="metadata" — Core Web Vitals; don't download the whole file
--   • playsinline     — iOS Safari plays in-flow (not fullscreen)
--   • muted (default) — many browsers block autoplay w/ audio; muted also
--                       stops iOS from auto-launching fullscreen
--
...
begin
  ...
  if v_destination = 'trip_report' then
    ...
    -- Card thumbnail: first IMAGE (not video) in the first photos block. Videos
    -- don't produce meaningful OG thumbnails, so skip them for the card.
    for v_section in select * from jsonb_array_elements(v_sections)
    loop
      if (v_section ->> 'type') = 'photos'
         and jsonb_typeof(v_sub.draft -> (v_section ->> 'id')) = 'array' then
    ...
    loop
      ...
      if v_st = 'p' then
        ...
        -- Inline media gallery: <figure> per entry.
        --   • Image  → <img alt="..." loading="lazy" />
        --   • Video  → <video controls preload="metadata" playsinline>
        -- Optional <figcaption> from entry.caption in either case.
        -- Skips _uploading placeholders and entries without a resolvable URL.
        v_arr := v_sub.draft -> (v_section ->> 'id');
        if jsonb_typeof(v_arr) = 'array' and jsonb_array_length(v_arr) > 0 then
          select string_agg(
                   '<figure>'
