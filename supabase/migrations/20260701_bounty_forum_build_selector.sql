-- ─────────────────────────────────────────────────────────────────────────────
-- admin_approve_bounty (forum_thread destination) — build_select section
-- ─────────────────────────────────────────────────────────────────────────────
-- Adds support for the new `build_select` form section: a participant picks
-- one of their builds when responding to the bounty, and the published
-- forum thread renders a "VIEW IN BUILD GALLERY" CTA card in the position
-- the admin placed the block.
--
-- Draft shape: plain build uuid string. The walker looks up the build's
-- name / year / make / model / hero_img at publish time so the CTA card
-- can display richer info + so the alt text serves SEO.
--
-- Emitted HTML (semantic anchor styled as a card via `.th-build-cta` in
-- the client's thRbCSS):
--
--   <a href="/builds/UUID" class="th-build-cta">
--     <img src="HERO" alt="BUILD NAME" loading="lazy" />
--     <span class="th-build-cta-body">
--       <span class="th-build-cta-eyebrow">VIEW IN BUILD GALLERY</span>
--       <strong>BUILD NAME</strong>
--       <span class="th-build-cta-meta">YEAR MAKE MODEL TRIM</span>
--     </span>
--     <span class="th-build-cta-arrow">→</span>
--   </a>
--
-- If the build no longer exists (deleted between claim and approve), the
-- section is silently skipped rather than producing a broken link.
--
  ...
  if v_destination = 'trip_report' then
    ...
    loop
      ...
      if v_st = 'p' then
        ...
        -- Draft value is a uuid string. Look up the build so the CTA
        -- carries the actual build name + hero for SEO / preview quality.
        -- If the build was deleted, silently skip (avoids broken links).
        v_build_id := null;
        v_build := null;
        begin
